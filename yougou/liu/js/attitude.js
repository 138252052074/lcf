...
//		console.log(scrollTop);
	...
	//	轮播图播放
	var oOnly=document.getElementById('only');
	var aOnlyLi=oOnly.getElementsByTagName('li');
	...
	function tab() {
		if(now>=aOnlyLi.length){
			now=0;
		}
		else if(now<0){
			now=aOnlyLi.length-1;
		}
		startMove(oOnly,{left:-960*now},30);
	}
